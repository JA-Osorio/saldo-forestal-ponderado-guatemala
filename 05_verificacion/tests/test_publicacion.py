...
local revisan el único cuaderno situado en ``04_reproduccion_python/``.
"""
...
DIRECTORIO_CUADERNOS = REPO / "04_reproduccion_python"
...
def _ruta_sometida_a_prueba() -> Path:
    ...
    assert len(activos) == 1, (
        "La publicación debe tener exactamente un cuaderno activo en "
...
def test_hay_un_unico_cuaderno_publico_activo():
    activos = _cuadernos_activos()
    assert len(activos) == 1, (
        "Debe existir exactamente un .ipynb publicable en 04_reproduccion_python/."
    )
...
def test_manifiesto_excluye_estructura_heredada():
    ruta = REPO / "05_verificacion" / "manifiesto_archivos.csv"
    with ruta.open(encoding="utf-8", newline="") as archivo:
        rutas = [fila["ruta"] for fila in csv.DictReader(archivo)]
    prefijos_heredados = (
        "data/",
        "docs/",
        "notebooks/",
    ...
    assert not [ruta for ruta in rutas if ruta.startswith(prefijos_heredados)]
    assert any(ruta.startswith("04_reproduccion_python/src/") for ruta in rutas)
    assert any(ruta.startswith("05_verificacion/tests/") for ruta in rutas)
