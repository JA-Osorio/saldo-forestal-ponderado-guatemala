...
Poorter et al. (2016) estiman la recuperación de biomasa aérea de bosques secundarios neotropicales veinte años después del abandono. El repositorio utiliza intervalos regionales para ponderar la recuperación, no como medición directa de las ganancias de cobertura reportadas en Guatemala.

Los municipios se asignan a cinco regiones de referencia según correspondencias ambientales documentadas. El dominio contiene 172 municipios; quedan fuera 168 municipios del altiplano, bosques montanos, coníferas u otras condiciones sin correspondencia defendible, además de las dos unidades lacustres.
...
El pipeline lee el valor unitario, el factor de homologación, la tasa central y
los horizontes directamente de
`data/raw/parametros_valoracion.csv`. La tabla
`parametros_valoracion_aplicados.csv` conserva los valores efectivamente usados
en cada ejecución. Las constantes del código son valores predeterminados para
funciones aisladas, no sustituyen el archivo de entrada en la cadena principal.
...
donde $m_s^B$ y $m_s^R$ modifican por separado la pérdida bruta y la recuperación. Esta formulación evita llamar “restauración” a un escenario que solo reduce $B$. En el pipeline, cualquier escenario denominado restauración debe aumentar explícitamente el multiplicador de recuperación.
...
El pipeline verifica, como mínimo:
...
Los resultados de cada ejecución se registran en `data/processed/controles_calidad.csv`. El manifiesto incluido en la descarga integral enumera tamaño y SHA-256 de cada miembro del ZIP, salvo el propio manifiesto para evitar una referencia circular.
